    ...
    with SessionLocal() as db:
        ...
        for code, rows in MEMBERS.items():
            ...
            for student_no, name, dept, grade in rows:
                exists = db.scalar(select(Member).where(Member.organization_id == org.id, Member.fiscal_year == fy, Member.student_no == student_no))
                if exists:
                    continue
                db.add(Member(organization_id=org.id, fiscal_year=fy, student_no=student_no, name=name, department=dept, grade=grade, registered_by="seed"))
                members_added += 1
        db.commit()
