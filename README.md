...
名簿は **ファイル添付ではなく、「学籍番号・氏名・所属・学年」の 4 列の表としてアプリに入力**します。

- 入力欄がこの 4 列しか無いので、病歴・障害情報・常用薬・保護者連絡先などの要配慮情報は物理的に登録できません。
  貼り付けた行が 4 列でなければエラーになります。
- 職員は活動届の画面でそのまま参加者を確認でき、ダッシュボードから「今この場所にいる団体」を横断検索できます。
- **年度部員名簿** (年 1 回) も同じ 4 列で登録します。団体の代表者・副代表者が Excel からコピーして貼り付けるだけです。
- **活動参加者名簿** は活動届フォームで、年度部員名簿の一覧にチェックを入れる + 部員以外を貼り付けで追加、の 2 通りで作れます。
  年度名簿を登録しておくと活動ごとの手間が大きく減ります (要件どおり、年度名簿があっても参加者名簿は省略できません)。
- 行程表・大会要項はこれまでどおりファイル添付です。
...
7. **鈴木 (登山部 副代表)** でログイン → 「年度部員名簿」→ 登山部 → 4 列を貼り付けて保存 → 5 列目に何か書いた行を混ぜるとエラーになる
...
│   │   ├── roster.py      # 名簿の貼り付け解析 (4 列だけを受け付ける)
