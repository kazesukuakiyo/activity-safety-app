...
def process_submission(db: Session, report: ActivityReport, *, resubmission: bool = False) -> None:
    ...
    if report.applicant_check != ApplicantCheck.MATCH:
        notify.send(
            ...
            body=(f"申請者 {applicant} <{report.applicant_email}> は団体台帳の代表者・副代表者と一致しません。\n"
    ...
    if report.requires_precheck and not missing:                  # 8. 確認依頼
        ...
        notify.send(db, to=settings.staff_notify_email, kind="staff_request", report_id=report.id,
                    subject=f"{subject_prefix} 事前確認依頼{' (再提出)' if resubmission else ''}", body=body)
    ...
    db.add(AuditLog(report_id=report.id, actor_email=report.applicant_email,
